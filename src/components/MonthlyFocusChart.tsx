...
import Svg, { G, Line, Text as SvgText, Defs, LinearGradient, Stop, Path, Rect } from 'react-native-svg';
...
function makeScale(values: number[], height: number) {
  ...
  const toY = (top: number) => (v: number) => PAD_T + (height - PAD_T - PAD_B) * (1 - v / top);
...
export default function MonthlyFocusChart({ data, focusColor = '#F46C6C', todayISO }: Props) {
  const [w, setW] = useState(0);
  const [h, setH] = useState(300);
  const { axisLabel, xAxisLabel, gridLine, gridOpacityMajor, gridOpacityMinor } = useChartColors();
        ...
        <Svg width={w} height={h}>
          ...
          <G>
            {scale.ticks.map((t, i) => {
              ...
              return (
                <G key={`tick-${i}`}>
                  <Line x1={PAD_L} x2={w - PAD_R} y1={y} y2={y} stroke={gridLine} opacity={major ? gridOpacityMajor : gridOpacityMinor} />
                  {!!scale.format(t) && (
                    <SvgText x={PAD_L - 8} y={y + 4} fontSize={TICK_TEXT_SIZE} fill={axisLabel} textAnchor="end">
                      {scale.format(t)}
                    </SvgText>
                  )}
                </G>
          ...
          <G>
            {data.map((d, idx) => {
              ...
              return (
                <G key={`bar-${idx}`}>
                  {isCurrentWeek && (
                    <Rect x={x - 6} y={topY - 6} width={barW + 12} height={focusH + 12} fill={focusColor} opacity={0.08} rx={BAR_RADIUS + 4} ry={BAR_RADIUS + 4} />
                  )}
                  <Path d={focusPathD} fill="url(#gradFocusM)" />
                  <SvgText x={x + barW / 2} y={h - 16} fontSize={11} fill={xAxisLabel} textAnchor="middle" fontWeight={isCurrentWeek ? '700' : '500'}>
                    {d.label}
                  </SvgText>
                </G>
              );
